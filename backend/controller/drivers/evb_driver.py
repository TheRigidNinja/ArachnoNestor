...
class EVBDriver:
    ...
    def snapshot(self, winch_id: int) -> Snapshot:
        r_winch, total_count, hall_raw, cache_age_ms = evb_api.get_snapshot(self.client, winch_id)
        return Snapshot(winch=r_winch, total_count=total_count, hall_raw=hall_raw, cache_age_ms=cache_age_ms)

    def bundle(self, winch_id: int) -> Bundle:
        b = evb_api.get_bundle(self.client, winch_id)
        return Bundle(
            winch=b["winch"],
            flags=b["flags"],
    ...
    def imu(self) -> Imu:
        ...
        return Imu(
            ...
            roll=i["roll"],
            yaw=i["yaw"],
            cache_age_ms=i["cache_age_ms"],
        )

    def distance(self) -> dict:
        return evb_api.get_distance(self.client)
