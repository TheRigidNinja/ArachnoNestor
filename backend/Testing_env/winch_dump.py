...
from evb_client import send_command


def get_snapshot(host: str, port: int, timeout: float, winch_id: int):
    resp_type, payload = send_command(host, port, timeout, 0x04, bytes([winch_id]))
    if resp_type == 0xE0:
...
def get_delta(host: str, port: int, timeout: float, winch_id: int):
    resp_type, payload = send_command(host, port, timeout, 0x05, bytes([winch_id]))
    if resp_type == 0xE0:
...
def get_distance(host: str, port: int, timeout: float, winch_id: int):
    """
    ...
    """
    resp_type, payload = send_command(host, port, timeout, 0x07, b"")
    if resp_type == 0xE0:
...
def get_power(host: str, port: int, timeout: float, winch_id: int):
    resp_type, payload = send_command(host, port, timeout, 0x08, bytes([winch_id]))
    if resp_type == 0xE0:
...
def get_bundle(host: str, port: int, timeout: float, winch_id: int):
    """
    ...
    """
    resp_type, payload = send_command(host, port, timeout, 0x09, bytes([winch_id]))
    if resp_type == 0xE0:
...
def get_imu(host: str, port: int, timeout: float):
    """
...
def poll_once(host: str, port: int, timeout: float, winches: List[int]):
    rows = []
    for w in winches:
        bundle = get_bundle(host, port, timeout, w)
        rows.append(bundle)
    return rows
...
def format_rows(rows):
    ...
    for r in rows:
        lines.append(
            f"winch={r['winch']} flags=0x{r['flags']:02X} total={r['total_count']} "
            f"delta={r['delta_count']} hall={r['hall_raw']} dist_mm={r['dist_mm']} "
            f"strength={r['strength']} temp_raw={r['temp_raw']} age_ms={r['age_ms']} "
...
def main():
    ...
    ap.add_argument("--winches", type=parse_winch_list, default=[0, 1, 2, 3],
                    help="comma-separated winch ids, e.g. 0,1,2,3")
    ...
    try:
        while True:
            iteration += 1
            rows = poll_once(args.host, args.port, args.timeout, args.winches)
            print(f"[{time.strftime('%H:%M:%S')}] poll #{iteration}")
            print(format_rows(rows))
            if not args.no_distance:
                try:
                    dist = get_distance(args.host, args.port, args.timeout, 0)
                    print(f"distance ok={dist['ok']} dist_mm={dist['dist_mm']} strength={dist['strength']} temp_raw={dist['temp_raw']} age_ms={dist['age_ms']}")
                except Exception as exc:
                    print(f"distance read failed: {exc}")
            if not args.no_imu:
                try:
                    imu = get_imu(args.host, args.port, args.timeout)
                    print(format_imu(imu))
                except Exception as exc:
